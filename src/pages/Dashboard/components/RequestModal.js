      ...
      <div className="modal-overlay"> 
        ...
        <div className="modal-content">
                      ...
                      <tr key={book.book_id}>
                        ...
                        <td>
                          <button 
                            ...
                            disabled={book.book_status === 'RETURNED'}
                            className={book.book_status === 'RETURNED' ? 'return-button disabled' : 'return-button'}
                          >
                            Return
                          </button>
                        </td>
          ...
          <button
                        ...
                        disabled={requestDetails?.status === 'Approved' || requestDetails?.status === 'Overdue'}
                        className={requestDetails?.status === 'Approved' ? 'Approved' : 'approve-button'}
                      >
                        {requestDetails?.status === 'Approved' ? 'Approved' : 'Approve'}
                      </button>
                      <button
                        onClick={() => handleDecline(requestDetails?.req_id)}
                        disabled={requestDetails?.status === 'Rejected' || requestDetails?.status === 'Overdue'}
                        className={requestDetails?.status === 'Rejected' ? 'Rejected' : 'reject-button'}
                      >
